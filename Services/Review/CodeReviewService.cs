...
public sealed class CodeReviewService : ICodeReviewService
{
    ...
    public async Task ReviewPullRequestAsync(WebhookPayload payload, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation(
            "Starting review for PR #{PrId} '{Title}' in {Workspace}/{Repo}",
        ...
        {
            var summary = BuildSummaryComment(allResults, pr.Title);
            try
            {
                await _bitbucket.PostSummaryCommentAsync(workspace, repoSlug, prId, summary, ct);
            }
            catch (Exception ex)
            {
        ...
        string prTitle)
    {
        ...
        foreach (var file in results.Where(r => r.Comments.Count > 0 || !string.IsNullOrWhiteSpace(r.Summary)))
        {
        ...
        sb.AppendLine("---");
        sb.AppendLine("*Generated by [BitbucketCodeReview](https://github.com/) using Claude AI.*");

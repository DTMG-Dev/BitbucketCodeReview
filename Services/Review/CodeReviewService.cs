...
public sealed class CodeReviewService
{
    ...
    private static string BuildSummary(IReadOnlyList<FileReviewResult> results, string prTitle)
    {
        ...
        if (total == 0)
            return $"🤖 **AI Review** — {results.Count} file(s) reviewed · ✅ No issues found.";

        var verdict = errors   > 0 ? "❌ Changes requested"
                    : warnings > 0 ? "⚠️ Review comments added"
                                   : "✅ Looks good";

        return $"🤖 **AI Review** — {results.Count} file(s) · {verdict} · " +
